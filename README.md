
# Afaq Ahmad Ali 👋

![status-badge](https://img.shields.io/badge/status-active-brightgreen) ![license-badge](https://img.shields.io/badge/license-MIT-blue)

Full Stack Developer (MERN) focused on building practical, user-centered web applications and integrating lightweight AI solutions.

🎓 Software engineer 

## Table of Contents
- [About](#about)
- [Featured Projects](#featured-projects)
- [Tech Stack](#tech-stack)
- [Learning](#learning)
- [Contact](#contact)
- [Contributing](#contributing)
- [License](#license)

## About
I build full-stack applications using React, Node.js, and MongoDB. I enjoy clean architecture, backend engineering, and applying simple ML/AI libraries (e.g., face-api.js) to solve real problems like focus monitoring and smart travel assistants.

## 🚀 Featured Projects

- **[Smart Travel Companion](https://github.com/Afaq-Ahmad-eng/Travel-Companion)** — Tourism-focused web app for KPK (routing, local content, trip planning).  

- **[FocusMonitor](https://github.com/Afaq-Ahmad-eng/FocusMonitor)** — AI-based focus and productivity monitoring system using browser-based face/attention detection.  

- **Software Synergy Solutions Website** — Company site with CMS and content workflows.  
  [![Live Demo]](https://synergyinnovations.tech/)


## Tech Stack

- **Languages:** 
    - ![C++](https://img.shields.io/badge/C++-blue)
    - ![JavaScript](https://img.shields.io/badge/JavaScript-yellow)
    - ![HTML](https://img.shields.io/badge/HTML-orange)
    - ![CSS](https://img.shields.io/badge/CSS-purple)
- **Libraries and Frameworks:** 
    - ![React](https://img.shields.io/badge/React-blue)
    - ![Node.js](https://img.shields.io/badge/Node.js-green)
    - ![Express](https://img.shields.io/badge/Express-black)

- **Databases:** 
    - ![MongoDB](https://img.shields.io/badge/MongoDB-green)
    - ![MySQL](https://img.shields.io/badge/MySQL-blue)

- **AI/ML:** 
    - ![face-api.js](https://img.shields.io/badge/face--api.js-lightgrey)

- **Other Technologies:** 
    - ![RESTful APIs](https://img.shields.io/badge/RESTful_APIs-blue)
    - ![JWT](https://img.shields.io/badge/JWT-yellow)

- **Deployment:** 
    - ![Vercel](https://img.shields.io/badge/Vercel-purple)
    - ![Netlify](https://img.shields.io/badge/Netlify-green)

- **Testing:** 
    - ![Playwright](https://img.shields.io/badge/Playwright-lightblue)

- **Tools:** 
    - ![Git](https://img.shields.io/badge/Git-black)
    - ![GitHub](https://img.shields.io/badge/GitHub-blue)
    - ![Postman](https://img.shields.io/badge/Postman-orange)
    - ![VS Code](https://img.shields.io/badge/VS_Code-purple)

## Learning
- Advanced backend architecture
- System design fundamentals
- Testing & automation

## Contact
- ![LinkedIn](https://img.shields.io/badge/LinkedIn-blue) [Ahmad Ali](https://www.linkedin.com/in/ahmad-ali)
- Email: afaqahmad2593@gmail.com
- ![Portfolio](https://img.shields.io/badge/Portfolio-purple) [My Portfolio](https://afaq-ahmad-eng-portfolio.vercel.app/)

## Contributing
Contributions, issues, and feature requests are welcome. For small fixes, open a pull request; for larger changes, please open an issue first to discuss.

## License
This project is licensed under the MIT License.

